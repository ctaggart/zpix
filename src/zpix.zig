...
pub fn loadFile(allocator: Allocator, path: []const u8, io: std.Io) !Image {
    ...
    const bytes_read = blk: {
        ...
        defer file.close(io);
        break :blk try file.readPositionalAll(io, &header, 0);
    };
    if (bytes_read < 2) return FormatError.UnsupportedFormat;

    const format = detectFormat(header[0..bytes_read]);
    switch (format) {
        .png => return png.loadFromFile(allocator, path, io),
        .jpeg => return jpeg.loadFromFile(allocator, path, io),
...
pub fn saveFile(img: *const Image, path: []const u8, io: std.Io) !void {
    const extension = std.fs.path.extension(path);
    if (std.ascii.eqlIgnoreCase(extension, ".png")) {
        return png.saveToFile(img, path, io);
    } else if (std.ascii.eqlIgnoreCase(extension, ".jpg") or std.ascii.eqlIgnoreCase(extension, ".jpeg")) {
        return jpeg_encoder.saveToFile(img, path, 90, io);
    } else {
...
test "loadFile loads PNG by magic bytes" {
    var img = try loadFile(std.testing.allocator, "test/fixtures/test_rgb_4x4.png", std.testing.io);
    defer img.deinit();
...
test "loadFile loads JPEG by magic bytes" {
    var img = try loadFile(std.testing.allocator, "test/fixtures/test_rgb_4x4.jpg", std.testing.io);
    defer img.deinit();
    try std.testing.expectEqual(@as(u32, 4), img.width);
    try std.testing.expectEqual(@as(u32, 4), img.height);
}
