...
test "JPEG: nonexistent file" {
    ...
    const result = zpix.loadJpegFile(allocator, "test/fixtures/does_not_exist.jpg", std.testing.io);
...
test "PNG: nonexistent file" {
    ...
    const result = zpix.loadPngFile(allocator, "test/fixtures/does_not_exist.png", std.testing.io);
    try std.testing.expectError(error.FileNotFound, result);
}
...
test "JPEG: directory instead of file" {
    ...
    const result = zpix.loadJpegFile(allocator, "test/fixtures", std.testing.io);
...
test "PNG: directory instead of file" {
    ...
    const result = zpix.loadPngFile(allocator, "test/fixtures", std.testing.io);
    // Should fail (either IsDir or some read error)
    try std.testing.expect(std.meta.isError(result));
}
